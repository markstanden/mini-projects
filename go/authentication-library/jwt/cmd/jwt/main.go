...
func run(args []string, stdout io.Writer) error {
	token, err := jwt.NewToken(secret, "My Server", "User Idenfier", "My Website", "Unique ID for the JWT", "Key ID (to identify secret version used to encrypt signature")
	if err != nil {
		fmt.Println("Error: ", err)
	}
	fmt.Println("Created Token: ", token)

	data, err := jwt.Validate(token, secret)
	if err != nil {
		fmt.Println("Error: ", err)
	}
